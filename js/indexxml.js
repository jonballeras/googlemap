...
.then(data => {
  ...
  sortSelect?.addEventListener('change', function handleChange() {
    ...
    for (let i = 0; i < dataProperties.length; i++) {
      dataProperties[i].parentNode.appendChild(dataProperties[i]);
    }
  });
...
function initData(data) {
  ...
  for (let i = 0; i < dataProperties.length; i++) {
      const id = dataProperties[i]?.children[7]?.textContent;
      const recId = dataProperties[i]?.attributes[1]?.nodeValue;
      const address = dataProperties[i]?.children[0]?.textContent;
      const city = dataProperties[i]?.children[3]?.textContent;
      const state = dataProperties[i]?.children[4]?.textContent;
      const postalCode = dataProperties[i]?.children[5]?.textContent;
      const neighborhood = dataProperties[i]?.children[13]?.textContent;
      const bathrooms = dataProperties[i]?.children[10]?.textContent;
      const bedrooms = dataProperties[i]?.children[11]?.textContent;
      const type = dataProperties[i]?.children[15]?.textContent;
      const status = dataProperties[i]?.children[18]?.textContent;
      const price = Number(dataProperties[i]?.children[17]?.textContent);
      const img = dataProperties[i]?.children[27]?.textContent;
      const locationData = dataProperties[i]?.children[2]?.textContent.split(",");
